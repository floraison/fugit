...
group Fugit::Cron do
  ...
  NEXT_TIMES = [
    ...
    # min hou dom mon dow, asserted next time[, now]
  ...
  group '#next_time' do
    ...
    NEXT_TIMES.each do |cron, next_time, now, zone_name|
      ...
      test(d) do
        ...
        in_zone(zone_name) do
          ...
          assert(nt.zone.name, zone_name) if zone_name
        end
      end
    end

    group 'implicit tz DST transition' do
      ...
      [
        ...
        test "correctly transit in or out of DST for #{tz.inspect}" do
          ...
          in_zone(tz) do
            ...
            assert(Fugit.time_to_zone_s(nt), target)
          end
        end
      end

      test 'correctly increments every minute into DST' do
      ...
      test 'correctly increments every minute into DST (explicit TZ)' do
      ...
      test 'correctly increments into DST (gh-53 a)' do
      ...
      test 'correctly increments into DST (gh-53 b)' do
      ...
      test 'correctly increments into DST (gh-53 c)' do
      ...
      test 'correctly increments into DST (gh-53 d)' do
      ...
      test 'correctly increments into DST (America/New_York) (gh-63)' do
      ...
      test 'correctly increments out of DST (gh-53 e)' do
      ...
      test 'correctly increments out of DST' do
        ...
        in_zone 'America/Los_Angeles' do
          ...
          assert(c.brute_frequency(2015).occurrences, 8759)
        end
      end

      test 'correctly increments out of DST (America/New_York)' do
        ...
        in_zone 'America/New_York' do
          ...
          assert(c.brute_frequency(2018).occurrences, 365)
        end
      end

      test 'correctly increments into DST (America/Santiago) gh-60' do
        ...
        in_zone 'America/Santiago' do
          ...
          assert c.brute_frequency(2021).occurrences, 12
        end
      end

      test 'correctly increments into DST (America/Santiago) Time.zone gh-62' do
      ...
      test 'correctly increments into DST (America/Santiago) Time.zone gh-62 hourly' do
        ...
        in_active_support_zone 'America/Santiago' do
          ...
          assert c.brute_frequency(2021).occurrences, 8759
        end
      end

      test 'correctly increments out of DST (America/Santiago) gh-60' do
      ...
      test 'correctly increments out of DST (America/Santiago) Time.zone gh-62' do
    ...
    test 'returns a plain second' do

      c = Fugit::Cron.parse('* * * * *')
      nt = c.next_time

      assert(nt.seconds.to_s, nt.seconds.to_i.to_s + '.0')
    end

    group 'explicit timezone' do

      test 'computes in the cron zone but returns in the from zone' do
        ...
        assert(t.iso8601, '2017-03-25T22:00:00+04:00')
      end

      test 'returns the right result' do
        ...
        assert t.class, EtOrbi::EoTime
        assert t.iso8601, '2018-01-01T03:00:00+04:00'
        assert t.translate('Europe/Rome').iso8601, '2018-01-01T00:00:00+01:00'
      end
    end

    test 'breaks if its loop takes too long' do
    ...
    group '(defective et-orbi)' do
      ...
      test 'breaks if its loop stalls' do
    ...
    group '(Chronic and ActiveSupport, gh-11)' do
      ...
      test "doesn't stall or loop ad infinitum" do

        in_active_support_zone('UTC') do

          cron = Fugit.do_parse_cron('0 0 1 1 *')

          assert_not_error { cron.next_time }
        end
      end
    end

    group 'New York skip (gh-43)' do

      test "doesn't skip" do
...
#p nt.utc.to_s
        ...
        assert nt.to_s, / 16:00:00 /
      end

      test "doesn't skip (TZ UTC)" do
...
#p nt.utc.to_s
          ...
          assert nt.utc.to_s, / 16:00:00 /
        end
      end

      test "doesn't skip (ActiveSupport TZ America/New_York)" do
...
#p nt.utc.to_s
      ...
      test 'does not break on "* * * * 1%2+2" (gh-47)' do

        cron0 = Fugit.parse('0 8 * * 1%2+2')
        cron1 = Fugit.parse('0 8 * * 1%2')

        assert(
          cron0.next_time('2021-04-21 07:00:00').to_s,
          /^2021-05-03 08:00:00 /)
        assert(
          cron0.next_time('2021-04-21 07:00:00').to_s,
          cron1.next_time('2021-04-21 07:00:00').to_s)
      end

      test 'does not break on "30 14 * * 4%4+3" (gh-76)' do
        ...
        #assert nt.to_s, /^2022-10-06 03:30:00 (Z|\+0000)$/
          # post gh-47
        assert nt.to_s, /^2022-09-29 04:30:00 (Z|\+0000)$/
          # post gh-76

        in_zone('Australia/Melbourne') do
          #assert(nt.to_t.to_s, '2022-10-06 14:30:00 +1100') # post gh-47
          assert(nt.to_t.to_s, '2022-09-29 14:30:00 +1000') # post gh-76
        end
      end
    end
  end
  ...
  group '#match?' do
    ...
    NEXT_TIMES.each do |cron, next_time, _, zone_name|
      ...
      test "succeeds #{cron.inspect} ? #{next_time.inspect}" do
    ...
    group '"0 0 * * * Europe/Berlin" (gh-31)' do # in Changi
      ...
      test "doesn't match midnight in London" do

        in_zone('Europe/London') do
          assert @cron.match?(Time.new(2019, 1, 1)), false
        end
      end

      test "matches midnight in Berlin" do

        in_zone('Europe/London') do
          assert @cron.match?(Fugit.parse('2019-1-1 00:00:00 Europe/Berlin'))
        end
      end
    end
  end
  ...
  group '#previous_time' do
    ...
    PREVIOUS_TIMES.each do |cron, previous_time, now, zone_name|
      ...
      test(d) do
        ...
        in_zone(zone_name) do
          ...
          assert(c.match?(ept), true) # quick check
        end
      end
    end
    ...
    test 'does not go into an endless loop over time == previous_time (gh-15)' do

      c = Fugit.parse('10 * * * * *')
      t = c.previous_time.to_f# + 0.123 #(some float x so that 0.0 <= x < 1.0)

      assert c.previous_time(Time.at(t)).to_i, t.to_i - 60
    end
  ...
  group '#brute_frequency' do
    ...
    [
      ...
      test "computes #{freq.inspect} for #{cron.inspect}" do
    ...
    test 'accepts a year argument' do
  ...
  group '#rough_frequency' do
              ...
              # day of week  0-7 (0 or 7 is Sun, or use names)
    {
      ...
      test "gets #{cron.inspect} and outputs #{freq.inspect}" do
...
#p Fugit::Duration.parse(rf).deflate.to_plain_s
  ...
  group '.parse' do
    ...
    test "returns the input immediately if it's a cron" do

      c = Fugit.parse('* * * * *'); assert(c.class, Fugit::Cron)

      c1 = Fugit::Cron.parse(c)

      assert(c1.class, Fugit::Cron)
      assert(c1.object_id, c.object_id)
    end

    test 'returns nil if test cannot parse' do

      assert(Fugit::Cron.parse(true), nil)
      assert(Fugit::Cron.parse('nada'), nil)
    end
    ...
    group 'success' do
      ...
      [
        ...
        [ '15-40/30 * * * *', '15 * * * *' ], #
        [ '0 4/12 * * *', '0 4,16 * * *' ],   #
        [ '0 8/4 * * *', '0 8,12,16,20 * * *' ],                  # gh-123
        [ '0 15/1 * * *', '0 15,16,17,18,19,20,21,22,23 * * *' ], # exploration
        [ '0 8-14/1 * * *', '0 8,9,10,11,12,13,14 * * *' ],       #
        ...
        test "parses #{c}" do
          ...
          assert(c ? c.to_cron_s : c, e)
        end
      }

      group 'negative monthdays' do
        ...
        [
          ...
          test("parses #{c}") { assert(Fugit::Cron.parse(c).to_cron_s, e) }
        }
      end
...
#a_e  q '0 0 * * mon#1,tue', [[0], [0], [0], nil, nil, [2], ["1#1"]]
      ...
      group 'timezone' do
        ...
        [
          ...
          test "returns nil for #{c.inspect}" do

            assert(Fugit::Cron.parse(c), nil)
          end
        }
      end
    end
    ...
    group 'failure' do
      ...
      [
        ...
        test "returns nil for #{cron.inspect}" do

          assert(Fugit::Cron.parse(cron), nil)
        end
      end
    end

    group 'impossible days' do

      {
        ...
        if modays
          ...
          test "parses #{cron.inspect} months/monthdays to #{modays.inspect}" do

            c = Fugit::Cron.parse(cron)

            assert([ c.months, c.monthdays ], modays)
          end
    ...
    group 'weekdays' do

      {
        ...
        test "parses #{cron.inspect} weekdays to #{weekdays.inspect}" do

          c = Fugit::Cron.parse(cron)

          assert(c.weekdays, weekdays)
        end
      end
    end
  end
  ...
  group '.do_parse' do
    ...
    [ #     min hou dom mon dow
      ...
      test "raises for #{cron}" do
        assert_error(
          lambda { Fugit::Cron.do_parse(cron) },
  ...
  group '#==' do
    ...
    test 'returns false else' do
      ...
      assert(
        Fugit::Cron.parse('* * * * *') == Fugit::Cron.parse('* * * * 1'),
        false)
      assert(
        Fugit::Cron.parse('* * * * *') != Fugit::Cron.parse('* * * * 1'),
        true)

      assert Fugit::Cron.parse('* * * * *') == 1, false
    end
  end
  ...
  group '#to_cron_s' do
    ...
    cron_s = {
      ...
      '0 0 12,13 * 1-5&' => '0 0 12,13 * 1,2,3,4,5&',
      '0 0 12,13& * 1-5' => '0 0 12,13 * 1,2,3,4,5&',
      '0 0 12,13& * 1-5&' => '0 0 12,13 * 1,2,3,4,5&',
    }
    ...
    cron_s.each do |source, target|
      ...
      test "represents #{source.inspect} into #{target.inspect}" do

        sc = Fugit::Cron.parse(source)
        tc = Fugit::Cron.parse(target)

        assert(sc, tc)
        assert(sc.to_cron_s, target)
      end
    end

    cron_s.each do |k, _|

      test "produces the same cron string if parsing (#{k.inspect} to_cron_s)" do
  ...
  group '#seconds' do
    ...
    }.each do |string, asserted|

      test "returns #{asserted.inspect} for #{string}" do

        assert(Fugit::Cron.parse(string).seconds, asserted)
      end
    end
  end

  group '#range (protected)' do

    {
      ...
      test "returns #{result.inspect} for #{args.inspect}" do

        as = [ :min, :max, :sta, :edn, :sla ].collect { |k| args[k] }

        assert(Fugit::Cron.allocate.send(:range, *as), result)
      end
    end
  end
end
...
group Fugit::Cron do
  ...
  group 'sec6' do
    ...
    [
      ...
      test "parses #{s0.inspect} and renders test as #{s1.inspect}" do

        c = Fugit::Cron.parse(s0)

        assert(c.to_cron_s, s1)
      end
    end
    ...
    [
      ...
      test "parses #{s.inspect} and stores test as #{a.inspect}" do
    ...
    [
      ...
      test "computes the #{nt ? 'next' : 'previous'} time correctly for #{cron.inspect}" do

        c = Fugit::Cron.parse(cron)

        if nt
          assert(Fugit.time_to_plain_s(c.next_time(from), false), nt)
        else
          assert(Fugit.time_to_plain_s(c.previous_time(from), false), pt)
        end
      end
    end
  end
  ...
  group '#within' do
    ...
    test 'returns all the occurrences within a given time period' do
    ...
    test 'returns all the occurrences within a start time and an end time' do
  ...
  group 'the module % extension' do

    # mostly about 2025-09-26 gh-114

    group 'EtOrbi.rweek_ref = :iso / :monday' do

      test 'runs "0 12 * * sun%2+1,wed%3+1" as Wed+Sun then 1w pause' do

        EtOrbi.rweek_ref = :monday # just to be sure

        assert(EtOrbi.rweek_ref, '2018-12-31')
    ...
    group 'EtOrbi.rweek_ref = :us / :sunday' do

      test 'runs "0 12 * * sun%2+1,wed%3+1" as Sun+Wed then 1w pause' do

        EtOrbi.rweek_ref = :sunday

        assert(EtOrbi.rweek_ref, '2018-12-30')
    ...
    test 'runs every fourth week on Tuesday' do
    ...
    test 'runs every fourth week and fourth week + 1 on Wednesday' do
